...
def _setup_parser():
    ...
    parser.add_argument('input_file', help='Path to file containing a list of newline-separated URLs of privacy policies.')
    parser.add_argument('--output_dir', '-o', default=CWD, help='Path to directory where policies will be saved (default is {}).'.format(CWD))
    parser.add_argument('--timeout', '-t', default=30, type=int, help='The amount of time in seconds to wait for response (default is 30).')
    ...
    parser.add_argument('--extractors', '-e', nargs='+', default=['text'], help='Extract information from the privacy policy using available extractors.')
    parser.add_argument('--workers', '-w', default=1, type=int, help='Number of threading workers to use (default is 1).')
    parser.add_argument('--force', '-f', action='store_true', help='Scrapes privacy policy again even if it was not previously updated (default is False).')
